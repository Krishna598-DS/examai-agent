    ...
    async with httpx.AsyncClient(
        ...
        try:
            ...
            if "text/html" not in content_type:
                logger.info(
                    "scraping_skipped_non_html", url=url, content_type=content_type
                )
                return None
            ...
            logger.warning(
                "scraping_http_error", url=url, status=e.response.status_code
            )
            return None
...
async def search_and_scrape(
    query: str, num_results: int = 3, scrape_top_n: int = 2
) -> str:
    """
